...
{ 
	...
	set_tss(CURR_TASK_TSS,(unsigned long)&TASK0.tss);
	set_ldt(CURR_TASK_LDT,(unsigned long)&TASK0.ldt);
	...
	/*Mouse_Install(X_Resolution,Y_Resolution);	
	Console();*/
	FATInfo ff;
	ff.op='s';
	...
	DrawRect(MainGraphicArea,&Color2,Draw_Normal ,200,200,150,150,3); 
	printf("\nHight=%d X_Resolution=%d",MainGraphicArea.hight,X_Resolution);
	getch();
	WindowGraphicMemoryCopy(MainGraphicArea,POINT(0 ,0),MainGraphicArea ,POINT(200 ,200) ,SIZE(150,150)); 
	printf("sdsdsdsdsdsd");
	getch();
	Console();
