		...
		else if(Strncmp(CMD,"dir",3)==0)
		{
			...
			for(Start=0;Start<TotalFrame;Start++)
			{
				...
				else
				{
					...
					strcat(Paramter2 ,CurrentDir[DriveNO] );
					strcat(Paramter2 ,CurDIR[Start].FileName );					
					strcat(Paramter2 ,"." );					
					Strncat(Paramter2 ,CurDIR[Start].FileExtention ,FILE_EXTENTION_SIZE );					
...
void DoFormat(char *Command ,char *Paramter1,int Start)
{
	...
	{		
		FormatPartition(mbr.SysName ,0 ,mbr.DiskPartition[0].SizeLBA ,mbr.DiskPartition[0].PartitionType ,(mbr.DiskPartition[0].bootable==BOOTABLE ? true:false));     
	}
...
void DoTeastWrite()
{
	...
	{
		FILE *fp;
		fp=fopen(Path,FOpenModeRDText|FOpenModeAppend);
		if(fp!=NULL)	
		{
