...
char *Strncpy(char *dest, const char *source, int count)//Note If source[i]=0 not Copy
{
  char *start = dest;
 ...
 char *Bytencpy(char *dest, const char *source, int count)//Copy All Byte Event 0
{
  char *start = dest;
...
char *Strncat (char *dest, const char *src,size_t MaxLen) {	
  ...
  memcpy (dest+DestLen ,src , MIN_NUM(strlen(src),MaxLen)+1);
  dest[DestLen+MIN_NUM(strlen(src),MaxLen)+1]=0;
  return dest;
}
