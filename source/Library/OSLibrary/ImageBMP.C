...
#define kernelMemCopy(src ,dest ,lengh) memcpy((void *)dest ,(void *)src ,lengh)

int detect(const char *fileName, char *dataPtr, int size/*,loaderFileClass *class*/)
{
  ...
  if (!Strncmp(dataPtr, "BM", 2))
    {
      ...
      //sprintf(class->className, "%s %s", FILECLASS_NAME_BMP ,FILECLASS_NAME_IMAGE);
...
int BMPLoad(unsigned char *imageFileData ,int dataLength , image *loadImage) 
{
  ...
  if ((imageFileData == NULL) || !dataLength || loadImage == NULL)
    return (status = -1/*ERR_NULLPARAMETER*/);
  ...
  imageData = (pixel *)kmalloc(loadImage->dataLength, "image data");
  if (imageData == NULL)
    {
      // Release the file data memory
      free((unsigned int *)imageFileData);
  ...
  if (header->bitsPerPixel == BMP_BPP_32BIT)
    {
      ...
      for (count1 = (height - 1); count1 >= 0; count1 --)
	  {
		...
		for (pixelRowCounter = 0; pixelRowCounter < width; pixelRowCounter++)
			{
			 imageData[pixelCounter].blue =imageFileData[fileOffset + (pixelRowCounter * 4)];
			 imageData[pixelCounter].green =imageFileData[fileOffset + (pixelRowCounter * 4) + 1];
			 imageData[pixelCounter++].red =imageFileData[fileOffset + (pixelRowCounter * 4) + 2];
			}
	  }
    }
      ...
      for (count1 = (height - 1); count1 >= 0; count1 --)
		{
		  ...
		  kernelMemCopy((imageFileData + fileOffset),(((unsigned int *) imageData)+((height - count1 - 1) * (width * 3))),(width * 3));
		}
    }
		...
		if (compression == BMP_COMP_NONE)
		{
			...
			for (count1 = (height - 1); count1 >= 0; count1 --)
			{
				...
				for (pixelRowCounter = 0; pixelRowCounter < width;pixelRowCounter++)
				{
					...
					{
						free((unsigned int *)imageFileData);
						free((unsigned int *)imageData);
						return (status = 0/*ERR_INVALID*/);
					}
			
					// Convert it to a pixel
					imageData[pixelCounter].blue = palette[colorIndex * 4];
					imageData[pixelCounter].green = palette[(colorIndex * 4) + 1];
					imageData[pixelCounter++].red = palette[(colorIndex * 4) + 2];
				}
			}
	       ...
	       for (pixelRowCounter = 0; pixelRowCounter < width; pixelRowCounter++)
			{
						...
						switch (imageFileData[fileOffset])
							{
								case 0:
									...
									case 2:
										// Code for delta.
										kprintf("RLE bitmap deltas not yet supported");
										free((unsigned int *)imageFileData);
										free((unsigned int *)imageData);
				...
				if (absoluteMode)
				{
					...
					for (count2 = 0; count2 < numBytes; count2++)
					{
						...
						if (colorIndex >= colors)
						{
							kprintf("Illegal color index %d", colorIndex);
							free((unsigned int *)imageFileData);
							free((unsigned int *)imageData);
...
//
		...
		else
		{
			...
			kprintf("Unsupported compression type %d", compression);
			free((unsigned int *)imageFileData);
			free((unsigned int *)imageData);
			return (status = -1/*ERR_INVALID*/);
		}
  ...
  loadImage->data = (unsigned int*)imageData;
...
int save(const char *fileName, image *saveImage)
{
  ...
  fileData =(unsigned char *)kmalloc(header.size,"Image File");
  if (fileData == NULL)
    {
      kprintf("Unable to allocate memory for bitmap file");
      return (status = 0/*ERR_MEMORY*/);
    }
  ...
  for (count = (saveImage->height - 1); count >= 0 ; count --)
    {
      kernelMemCopy(((unsigned int)saveImage->data + (count * (saveImage->width * 3))) ,imageData , (saveImage->width * 3) );
      MemClear((imageData + (saveImage->width * 3)), padBytes);
  ...
  //status = kernelFileOpen(fileName, (OPENMODE_WRITE | OPENMODE_TRUNCATE | OPENMODE_CREATE), &theFile);
  theFile=fopen(fileName ,FOpenModeRDWRBinary); 
  if (theFile==NULL)
    {
      kprintf("Unable to open %s for writing", fileName);
      // Free the file data
      free((unsigned int *)fileData);
      return (status);
    }

  // Write the file
  //status = kernelFileWrite(&theFile, 0, (header.size / theFile.blockSize) + ((header.size % theFile.blockSize)? 1 : 0), fileData);
  fwrite((char *)fileData ,1 ,header.size /*(header.size / theFile.blockSize) + ((header.size % theFile.blockSize)? 1 : 0)*/,theFile);
  ...
  if (status < 0)
    {
      kprintf("Unable to write %s", fileName);
      return (status);
    }
