...
{
  sprintf((unsigned char*)s, "%02x:%02x:%02x:%02x:%02x:%02x",
...
{
  struct pcnet32 *pcnet32 =(struct pcnet32 *) dev->privdata;
  int len;
  int left;
  struct pbuf *q;
...
int install(struct unit *unit)
{
  ...
  irq_install_handler((int)IRQ2INTR(pcnet32->irq),(void (*)(struct regs*)) pcnet32_handler);//automatic Enable IRQ
