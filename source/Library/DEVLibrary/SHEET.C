...
PSHTCTL shtctl_init(PMEMMAN memman, unsigned char *vram, int xsize, int ysize)
{
  PSHTCTL ctl;
  int i;
  
  ctl = (PSHTCTL) memman_alloc_4k(memman, sizeof(SHTCTL));
  if (ctl == 0)
    return ctl;
...
PSHEET sheet_alloc(PSHTCTL ctl)
{
  PSHEET sht;
  int i;
  for (i = 0; i < MAX_SHEETS; i++) {
...
void sheet_setbuf(PSHEET sht, unsigned char *buf, 
...
void sheet_refreshmap(PSHTCTL ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
...
void sheet_refreshsub(PSHTCTL ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
  int h, bx, by, vx, vy, bx0, by0, bx1, by1;
...
void sheet_updown(PSHEET sht, int height)
{
  PSHTCTL ctl = sht->ctl;
...
void sheet_refresh(PSHEET sht, int bx0, int by0, int bx1, int by1)
{  
  if (sht->height >= 0) {  
...
void sheet_slide(PSHEET sht, int vx0, int vy0)
{
