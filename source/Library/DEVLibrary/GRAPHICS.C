...
char _signiture1[4]={'V','E','S','A'};
char _signiture2[4]={'V','B','E','2'};
...
int RGBDrawPixel(unsigned short *VideoBuffer,ColorRGB *foreground,DrawMode mode, int xCoord, int yCoord)
{
  ...
  // Make sure the pixel is in the buffer
  if (((unsigned int)xCoord >= MainGraphicArea.width) || ((unsigned int)yCoord >= MainGraphicArea.hight))
    // Don't make an error condition, just skip it
    return (status = -1);

  // Make sure we're not drawing off the screen
  if ((xCoord < 0) || ((unsigned int)xCoord >= MainGraphicArea.width) ||
      (yCoord < 0) || ((unsigned int)yCoord >= MainGraphicArea.hight))
    return (status = -1);
...
bool GetSVGADetail(char *_info_buffer)
{
	...
	SVGADetail->inc=(unsigned char*)i386LinearToFp(SVGADetail->inc);//convert real address[ that generate in real mode] to the protected mode that each point to same location of ram[tell liner address]
	SVGADetail->table=(unsigned short*)i386LinearToFp(SVGADetail->table);
...
found:	
	...
	SVGA1->func=(void (*)(void))i386LinearToFp((void *)SVGA1->func);		//Fix Address
...
{	
	...
	if(G_PTR.BytePerPixel<=1)
		return false;
	unsigned short TypeSize=(unsigned short)ceill(G_PTR.BytePerPixel/2);
	unsigned int Y;		
...
{	
	//printf("MDest_PTR.Buffer=%4X",MDest_PTR.Buffer);getch();
	...
	//printf("DP(%d,%d) SP(%d,%d) S(%d,%d)",DestPoint.X ,DestPoint.Y,SrcPoint.X ,SrcPoint.Y ,WindowSize.Width ,WindowSize.Hight  );getch();
	
	//printf("Src=%4X,Dest=%4X Src_WordPerLine=%d,Dest_WordPerLine=%d TypeSize=%d",Src,Dest,Src_WordPerLine,Dest_WordPerLine,TypeSize);getch();
	...
	{		
		...
		//printf("S=%4X D=%4X\r",Src+MSrc_PTR.Buffer,Src+MSrc_PTR.Buffer);getch();
	  Src+=Src_WordPerLine;
