...
int DrawOval(GraphicArea G_PTR,ColorRGB  *foreground, DrawMode mode, int centerX, int centerY, int width,int height, int thickness, int fill)
{
  ...
  outerBitmap = kmalloc((outerRadius + 1) * sizeof(int),"DrawOval: OuterBitmap");
  if (outerBitmap == NULL)
  ...
  if ((thickness > 1) && !fill)
    {
      ...
      innerBitmap = kmalloc((innerRadius + 1) * sizeof(int),"DrawOval: Inner Bitmap");
      if (innerBitmap == NULL)
	return (status = -1/*ERR_MEMORY*/);
    }
...
static int KernelDrawImage(GraphicArea G_PTR ,image *drawImage ,DrawMode mode ,int xCoord ,int yCoord,unsigned int xOffset ,unsigned int yOffset ,int width ,int height)
{
  ...
  if ((int)(xCoord + lineLength) >= G_PTR.width)
  ...
  if ((yCoord + numberLines) >= G_PTR.hight)
  ...
  imageData = (pixel *)(drawImage->data + (((yOffset * drawImage->width) + xOffset) * 3));
...
{	
	...
	if( (width+xCoord)>G_PTR.width )
		width=G_PTR.width-xCoord;//Max availabe width
	if( (height+yCoord)>G_PTR.hight )
		height=G_PTR.hight-yCoord;//Max availabe height
	

	X_Repeat=ceill(width/(drawImage->width));
	Y_Repeat=ceill(height/(drawImage->height));
