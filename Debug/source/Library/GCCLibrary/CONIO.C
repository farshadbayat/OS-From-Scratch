...
/* INNER DATA */
unsigned int X_Resolution=80,Y_Resolution=25;

short    _cursor_x=0, _cursor_y=0;
int      _scrolling=1;
unsigned short *textmemptr;
char     _textattr=7;
char     _keybuf[_KEYBUF_SIZE];
int      _kbhead=0, _kbtail=0, _kbcnt=0;
short	 _StartLine=0;
ColorRGB GBackColor=COLOR_BLACK; 
...
static ColorRGB CTColorToGColor(char TColor)//convert Text Color To Graphic Color
{
	switch(TColor)
		{
		case BLACK:	
...
void scroll(void)
{
    ...
    *  backcolor too */
    ...
    if(_cursor_y >= Y_Resolution && IsGraphic==false)
    {
	...
	else if(((CHARROWS+SPACE_BETWEN_LINE)*_cursor_y) >= Y_Resolution && IsGraphic==true)
    {		
...
void clrscr()
{
    ...
    for(i = 0; i < Y_Resolution; i++)
		if(IsGraphic==false)
			...
			memsetw (MainGraphicArea.Buffer + i * X_Resolution, 0, X_Resolution);
...
void putch(unsigned char c)
{
    ...
    *  Index = [(y * width) + x] */
    else if(c >= ' ')
    {
		...
		if(IsGraphic==true)
		{
			...
			drawChar(MainGraphicArea ,(CHARROWS+SPACE_BETWEN_LINE)*_cursor_x ,(dword)((CHARCOLS+SPACE_BETWEN_CHAR)*_cursor_y) ,c-32,Color ,true);
		}
		else
    ...
    *  insert a new line in there */
    if(_cursor_x >= X_Resolution && IsGraphic==false)
    {
        _StartLine=_cursor_x = 0;
        _cursor_y++;
    }
