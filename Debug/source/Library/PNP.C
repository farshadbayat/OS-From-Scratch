...
{
	return pnp_bios_call_asm(func ,arg1 ,arg2 ,arg3 ,arg4 ,arg5 ,arg6 ,arg7 ,(char *)pnp_bios_thunk);
...
static int pnp_bios_dev_node_info(struct pnp_dev_node_info *data)
{
  ...
  gdt_set_gate(GDT_AUX1 ,(unsigned long) data ,sizeof(struct pnp_dev_node_info) ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
static int pnp_bios_get_dev_node(unsigned char *nodenum, char boot, struct pnp_bios_node *data)
{
 ...
 // seginit(&syspage->gdt[GDT_AUX2], (unsigned long) data, 64 * 1024, D_DATA | D_DPL0 | D_WRITE | D_PRESENT, 0);
  gdt_set_gate(GDT_AUX1 ,(unsigned long) nodenum ,sizeof(char) ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
  gdt_set_gate(GDT_AUX2 ,(unsigned long) data ,64 * 1024 ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
static int pnp_bios_isapnp_config(struct pnp_isa_config_struc *data)
{
  ...
  gdt_set_gate(GDT_AUX1 ,(unsigned long) data ,sizeof(struct pnp_isa_config_struc) ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
static int pnp_bios_read_escd(void *data, void *nvram_base)
{
  ...
  gdt_set_gate(GDT_AUX1 ,(unsigned long) data ,64 * 1024 ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
  gdt_set_gate(GDT_AUX2 ,(unsigned long) nvram_base ,64 * 1024 ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
...
void pnpid32_to_pnpid(unsigned long id, char *str)
{
...
static void extract_node_resource_data(struct pnp_bios_node *node, struct unit *unit)
{
  ...
  while (p < end) 
  {
	...
	{
	  int len = *(short *) &p[1];
	  unit->productname = (char *) malloc(len + 1);
...
static void build_sys_devlist(struct bus *bus)
{
  ...
  node = (struct pnp_bios_node *)malloc(info.max_node_size);
  if (!node) return;
...
{	
  ...
  while (more) 
  {
    ...
    more = b[i] & 0x80;
    datasize = b[i + 1] & 0x03;
    decodesz =(unsigned int)(b[i + 1] >> 2) & 0x03;
    // bit 4-7: reserved
...
static int build_isa_devlist(struct bus *bus)
{
  ...
  escd = (unsigned char *)malloc(escdinfo.escd_size);
  if (!escd) return 0;//-ENOMEM;
...
int enum_isapnp(struct bus *bus)
{
  ...
  for (ofs = 0xF0000; ofs < 0xFFFF0; ofs += 16)
  {
	...
	gdt_set_gate(GDT_PNPTEXT  ,pnpbios.pm16cseg ,64 * 1024 ,GDT_TO_SEL(GDT_TEXT) | D_DPL0 | D_READ | D_PRESENT, 0);
	gdt_set_gate(GDT_PNPDATA  ,pnpbios.pm16cseg ,64 * 1024 ,GDT_TO_SEL(GDT_DATA) | D_DPL0 | D_WRITE | D_PRESENT, 0);
	gdt_set_gate(GDT_PNPTHUNK ,(unsigned long) pnp_bios_thunk ,1 ,GDT_TO_SEL(GDT_TEXT) | D_DPL0 | D_READ | D_PRESENT, D_BIG | D_BIG_LIM);
    ...
    *((unsigned short *)(pnp_bios_thunk + 8)) = SEL_PNPTEXT;
	...
	gdt_set_gate(GDT_PNPTEXT  , 0, 0, 0, 0);
	gdt_set_gate(GDT_PNPDATA  , 0, 0, 0, 0);
	gdt_set_gate(GDT_PNPTHUNK , 0, 0, 0, 0);
