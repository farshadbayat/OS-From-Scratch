...
#define kernelMemCopy(src ,dest ,lengh) memcpy(dest ,src ,lengh)

int detect(const char *fileName, void *dataPtr, int size/*,loaderFileClass *class*/)
{
...
int BMPLoad(unsigned char *imageFileData ,int dataLength , image *loadImage) 
{
  ...
  imageData = kmalloc(loadImage->dataLength, "image data");
  if (imageData == NULL)
    {
      ...
      for (count1 = (height - 1); count1 >= 0; count1 --)
		{
		  ...
		  kernelMemCopy((imageFileData + fileOffset),(((void *) imageData)+((height - count1 - 1) * (width * 3))),(width * 3));
		}
    }
...
int save(const char *fileName, image *saveImage)
{
  ...
  fileData = kmalloc(header.size,"Image File");
  if (fileData == NULL)
    {
  ...
  for (count = (saveImage->height - 1); count >= 0 ; count --)
    {
      kernelMemCopy((saveImage->data + (count * (saveImage->width * 3))) ,imageData , (saveImage->width * 3) );
  ...
  fwrite(fileData ,1 ,header.size /*(header.size / theFile.blockSize) + ((header.size % theFile.blockSize)? 1 : 0)*/,theFile);
